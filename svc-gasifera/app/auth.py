"""Autenticación de svc-gasifera para el panel preliminar de solo lectura
(spec-sync-gasifera-pit.md §12, ADR-015).

- Valida el JWT de Firebase que reenvía el API Gateway en X-Forwarded-Authorization
  (idéntico a svc-vivienda/svc-privada).
- Resuelve rol + secretarías consultando portal_usuarios. Esa tabla vive en db_vivienda,
  así que svc-gasifera NO se conecta a esa base: llama a un endpoint interno IAM-only de
...
Roles de Gasífera mapean 1:1 a la jerarquía del portal (mismo criterio que svc-privada) —
no es un rol acotado como TecnicoDGV/Autoridad.
"""
import time
...
async def _fetch_portal_user(email: str) -> dict | None:
    ...
    igual criterio que app/auth.py de svc-vivienda/svc-privada).
    """
...
def require_gasifera(*roles: str):
    """Además del rol, exige pertenecer a la secretaría 'gasifera' (o ser Admin)."""
    ...
    async def check(user: AuthUser = Depends(get_current_user)) -> AuthUser:
        ...
        if user.role != "Admin" and SECRETARIA not in user.secretarias:
            raise HTTPException(
                ...
                detail={
                    "code": "PERMISO_INSUFICIENTE",
                    "message": "El usuario no tiene asignada la secretaría 'gasifera'.",
                },
            )
        return user
