"""Autenticación de svc-gralgob para el panel preliminar de solo lectura
(spec-sync-atp-compromiso-gobernador.md §12, ADR-015).

- Valida el JWT de Firebase que reenvía el API Gateway en X-Forwarded-Authorization
  (idéntico a svc-vivienda/svc-privada/svc-gasifera).
- Resuelve rol + secretarías consultando portal_usuarios. Esa tabla vive en db_vivienda,
  así que svc-gralgob NO se conecta a esa base: llama a un endpoint interno IAM-only de
...
async def _fetch_portal_user(email: str) -> dict | None:
    ...
    igual criterio que app/auth.py de svc-vivienda/svc-privada/svc-gasifera).
    """
...
def require_gralgob(*roles: str):
    """Además del rol, exige pertenecer a la secretaría 'gralgob' (o ser Admin)."""
    ...
    async def check(user: AuthUser = Depends(get_current_user)) -> AuthUser:
        ...
        if user.role != "Admin" and SECRETARIA not in user.secretarias:
            raise HTTPException(
                ...
                detail={
                    "code": "PERMISO_INSUFICIENTE",
                    "message": "El usuario no tiene asignada la secretaría 'gralgob'.",
                },
            )
        return user
